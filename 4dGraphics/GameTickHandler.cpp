...
void GameTickHandler::OnTick(void *_FData) {
  ...
  ); }; const auto KeyPressed = []( char c ) { return
  ImGui::IsKeyPressed(ImGuiASCIIidx(c),false); };
  */
  ...
  lightForce = glm::clamp( lightForce + dt * (KeyDown( 'P' ) ? 1 : -1),
  0.f, 1.f );
...

  {
          ...
          {
                  eulerCameraRot += vec2( -mouseDiff.x, mouseDiff.y ) *
  0.002f;

                  while( eulerCameraRot.x > glm::pi<float>() )
  eulerCameraRot.x -= 2 * glm::pi<float>(); while( eulerCameraRot.x <
  -glm::pi<float>() ) eulerCameraRot.x += 2 * glm::pi<float>();

                  eulerCameraRot.y = glm::clamp( eulerCameraRot.y,
  -glm::pi<float>() / 2, glm::pi<float>() / 2 );

                  CamRot = glm::angleAxis( eulerCameraRot.x, vUp ) *
  glm::angleAxis( eulerCameraRot.y, vRight ) * glm::identity<quat>();
          }
  }
  ...
  if( ShowDebugWindow )
  {
          ...
          {
                  ImGui::Text( "%.3f FPS (%.3f ms)", io.Framerate, 1000 /
  io.Framerate );
          }
          ImGui::End();
  }
